...
func (m *DefaultModelMapper) UpdateMappings(mappings []config.AmpModelMapping) {
	...
	for _, mapping := range mappings {
		...
		if mapping.Regex {
			...
			if len(targets) > 1 {
				log.Debugf("amp model regex mapping registered: /%s/ -> %v (chain)", from, targets)
			} else {
			...
			if len(targets) > 1 {
				log.Debugf("amp model mapping registered: %s -> %v (chain)", from, targets)
			} else {
	...
	if exactCount > 0 {
		log.Infof("amp model mapping: loaded %d exact mapping(s)", exactCount)
	}
	if regexCount > 0 {
		log.Infof("amp model mapping: loaded %d regex mapping(s)", regexCount)
	}
}
